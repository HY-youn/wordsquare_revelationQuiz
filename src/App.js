...
const BIBLE_VERSES = {
  revelation: {
    ...
    4: {
      ...
      6: "보좌 앞에 수정과 같은 유리바다가 있고 보좌 가운데와 보좌 주위에 네 생물이 있는데 앞 뒤에 눈이 가득하더라",
    ...
    5: {
      ...
      8: "책을 취하시매 네 생물과 이십 사 장로들이 어린양 앞에 엎드려 각각 거문고와 향이 가득한 금대접을 가졌으니 이 향은 성도의 기도들이라",
    ...
    6: {
      1: "내가 보매 어린양이 일곱 인 중에 하나를 떼시는 그 때에 내가 들으니 네 생물 중에 하나가 우뢰소리같이 말하되 오라 하기로",
    ...
    10: {
      ...
      4: "일곱 우뢰가 발할 때에 내가 기록하려고 하다가 들으니 하늘에서 소리나서 말하기를 일곱 우뢰가 발한 것을 인봉하고 기록하지 말라 하더라",
    ...
    16: {
      ...
      3: "둘째가 그 대접을 바다에 쏟으매 바다가 죽은 자의 피같이 되니 바다 가운데 모든 생물이 죽더라",
      ...
      10: "다섯째가 그 대접을 짐승의 보좌에 쏟으니 그 나라가 곧 어두워지며 사람들이 아파서 자기 혀를 깨물고",
    ...
    18: {
      ...
      11: "땅의 상고들이 그를 위하여 울고 애통하는 것은 다시 그들의 상품을 사는 자가 없음이라",
    ...
    21: {
      ...
      14: "그 성에 성곽은 열두 기초석이 있고 그 위에 어린양의 십 이 사도의 열 두 이름이 있더라",
  ...
  return (
    <div className="bible-quiz">
      ...
      <div className="answers-container">
        {answers.map((answer, index) => (
          <div key={index} className="answer-input">
            <label>빈칸 ({index + 1}):</label>
            <input
              type="text"
              value={userAnswers[index]}
              onChange={(e) => handleAnswerChange(index, e.target.value)}
              placeholder={`빈칸 ${index + 1}의 답을 입력하세요`}
            />
          </div>
        ))}
      </div>
      ...
      <div className="button-container">
        ...
        <button onClick={() => createQuiz(verse.text)}>새로운 퀴즈</button>
      </div>
            ...
            <p key={index}>
              ...
              {result.isCorrect ? (
                ...
                <span className="incorrect">
                  오답 (입력: {result.userAnswer}, 정답: {result.correctAnswer})
                </span>
              )}
            </p>
          ))}
